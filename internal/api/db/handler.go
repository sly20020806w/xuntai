...
func (h handler) listInstances(c *gin.Context) {
	...
	for _, row := range rows {
		out = append(out, gin.H{
			...
			"masterId": row.MasterID, "masterName": masters[row.MasterID], "running": false,
		})
	}
...
func (h handler) createInstance(c *gin.Context) {
	...
	row := model.Instance{
		...
		Version: version, Role: role, MasterID: masterID,
	}
	if err := h.deps.DB.Create(&row).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "实例没有建成"})
		return
	}
	c.JSON(http.StatusOK, gin.H{"id": row.ID, "name": row.Name, "role": row.Role, "running": false})
}
